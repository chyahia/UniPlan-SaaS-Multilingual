...
def run_tabu_search(
    ...
    """
    تنفيذ خوارزمية البحث المحظور (Tabu Search) مع استراتيجية موجهة بالأخطاء والأداء المحسن.
    تتضمن هذه النسخة تقنية (Apply/Undo) بـ O(1)، وذاكرة ديناميكية، وإصلاح ثغرة كسر الحلقات المتداخلة (found_in_level).
    """
    log_q.put("--- بدء البحث المحظور (النسخة النهائية الموجهة والأسرع أداءً) ---")
    ...
    unplaced, hard, soft_sum, soft_count = -best_fitness[0], -best_fitness[1], -best_fitness[2], -best_fitness[3]
    log_q.put(f"البحث المحظور: اللياقة الأولية (نقص, صارم, مرن) = ({unplaced}, {hard}, {soft_count}={soft_sum})")
    
    # --- الذاكرة الديناميكية (Dynamic Tenure) ---
    base_tenure = int(tabu_tenure)
    ...
    for i in range(max_iterations):
        ...
        # طفرة الركود (Stagnation Mutation)
        ...
        if (i + 1) % 50 == 0 and i > 0:
            unplaced, hard, soft_sum, soft_count = -best_fitness[0], -best_fitness[1], -best_fitness[2], -best_fitness[3]
            log_q.put(f"--- (متابعة) دورة {i+1}: أفضل لياقة حالية (ن,ص,م)=({unplaced}, {hard}, {soft_count}={soft_sum}) | الذاكرة: {current_tenure} ---")
        
        time.sleep(0)
        
        if best_fitness == (0, 0, 0, 0):
        ...
        scheduled_ids = {lec.get('id') for grid in current_solution.values() for day in grid for slot in day for lec in slot}
        ...
        move_to_add_to_tabu = None
        
        if unplaced_lectures:
            primary_targets = unplaced_lectures
            secondary_targets = hard_error_lecs or soft_error_lecs or all_lectures
        elif hard_error_lecs:
            primary_targets = hard_error_lecs
            secondary_targets = soft_error_lecs or all_lectures
        else:
            primary_targets = soft_error_lecs or all_lectures
            secondary_targets = all_lectures

        # --- بداية خوارزمية التقييم السريع (Apply & Undo) ---
        for _ in range(neighborhood_size):
            if random.random() < 0.70:
                lec_to_move = random.choice(primary_targets)
            else:
                lec_to_move = random.choice(secondary_targets)
            ...
            # 1. البحث والإزالة المؤقتة مع الإغلاق الذكي (found_in_level)
            removed_from = []
            old_day_tabu, old_slot_tabu = None, None
            
            for level_name in lec_to_move.get('levels', []):
                if level_name in current_solution:
                    found_in_level = False # علم للخروج السريع
                    for d_idx in range(len(current_solution[level_name])):
                        if found_in_level: break # كسر حلقة الأيام
                        for s_idx in range(len(current_solution[level_name][d_idx])):
                            if found_in_level: break # كسر حلقة الحصص
                            
                            slot_lecs = current_solution[level_name][d_idx][s_idx]
                            for lec in slot_lecs:
                                if lec.get('id') == lec_id_to_move:
                                    removed_from.append((level_name, d_idx, s_idx, lec))
                                    old_day_tabu, old_slot_tabu = d_idx, s_idx
                                    found_in_level = True # تم العثور عليها!
                                    break # كسر حلقة المحاضرات

            # تطبيق الحذف المؤقت
            for (level_name, d_idx, s_idx, original_lec) in removed_from:
                current_solution[level_name][d_idx][s_idx] = [l for l in current_solution[level_name][d_idx][s_idx] if l.get('id') != lec_id_to_move]

            # 2. إضافة المحاضرة مؤقتاً في المكان الجديد
            lec_with_new_room = lec_to_move.copy()
            lec_with_new_room['room'] = new_room
            added_levels = []
            for level_name in lec_to_move.get('levels', []):
                if level_name in current_solution:
                    current_solution[level_name][new_day_idx][new_slot_idx].append(lec_with_new_room)
                    added_levels.append(level_name)

            # 3. حساب اللياقة للجدول المُعدل جزئياً
            neighbor_fitness, _ = calculate_fitness(current_solution, all_lectures, days, slots, teachers, rooms_data, levels, identifiers_by_level, special_constraints, teacher_constraints, distribution_rule_type, lectures_by_teacher_map, globally_unavailable_slots, saturday_teachers, teacher_pairs, day_to_idx, rules_grid, last_slot_restrictions, level_specific_large_rooms, specific_small_room_assignments, constraint_severities=constraint_severities, use_strict_hierarchy=use_strict_hierarchy, max_sessions_per_day=max_sessions_per_day, consecutive_large_hall_rule=consecutive_large_hall_rule, prefer_morning_slots=prefer_morning_slots, non_sharing_teacher_pairs=non_sharing_teacher_pairs)

            # التحقق من الحظر
            is_tabu = False
            if old_day_tabu is not None:
                if (lec_id_to_move, new_day_idx, new_slot_idx) in tabu_list:
                    is_tabu = True

            # معيار الطموح والتحديث
            if not is_tabu or neighbor_fitness > best_fitness:
                ...
                is_better_neighbor = (neighbor_unplaced < best_neighbor_unplaced) or \
                                     (neighbor_unplaced == best_neighbor_unplaced and neighbor_hard < best_neighbor_hard) or \
                                     (neighbor_unplaced == best_neighbor_unplaced and neighbor_hard == best_neighbor_hard and neighbor_fitness > best_neighbor_fitness)
                
                if is_better_neighbor:
                    best_neighbor_fitness = neighbor_fitness
                    # تضمين removed_from هنا سيمنعنا من إعادة البحث عن المادة في الحلقة الدائمة O(1)
                    best_move_params = (lec_to_move, new_day_idx, new_slot_idx, new_room, removed_from)
                    move_to_add_to_tabu = (lec_id_to_move, old_day_tabu, old_slot_tabu)

            # 4. التراجع عن التغيير (Undo)
            for level_name in added_levels:
                current_solution[level_name][new_day_idx][new_slot_idx] = [l for l in current_solution[level_name][new_day_idx][new_slot_idx] if l.get('id') != lec_id_to_move]
            for (level_name, d_idx, s_idx, original_lec) in removed_from:
                current_solution[level_name][d_idx][s_idx].append(original_lec)
        # --- نهاية خوارزمية التقييم السريع ---

        # معالجة حافة الركود: ماذا لو لم نجد أي حركة مسموحة؟
        if best_move_params is None:
            stagnation_counter += 1
            if stagnation_counter > 5:
                current_tenure = min(current_tenure + 1, base_tenure * 3)
            continue

        # --- تطبيق أفضل حركة بشكل دائم (Permanent Apply) بـ O(1) ---
        lec_to_move, new_day_idx, new_slot_idx, new_room, removed_from_coords = best_move_params
        lec_id_to_move = lec_to_move['id']
        
        # الحذف المباشر O(1) باستخدام الإحداثيات المخزنة
        for (level_name, d_idx, s_idx, _) in removed_from_coords:
            current_solution[level_name][d_idx][s_idx] = [l for l in current_solution[level_name][d_idx][s_idx] if l.get('id') != lec_id_to_move]
                    
        # الإضافة الدائمة للمكان الجديد
        lec_with_new_room = lec_to_move.copy()
        lec_with_new_room['room'] = new_room
        for level_name in lec_to_move.get('levels', []):
            if level_name in current_solution:
        ...
        # تحديث قائمة الحظر
        if move_to_add_to_tabu and move_to_add_to_tabu[1] is not None:
            tabu_list.append(move_to_add_to_tabu)
        ...
        if current_fitness > best_fitness:
            ...
            unplaced, hard, soft_sum, soft_count = -best_fitness[0], -best_fitness[1], -best_fitness[2], -best_fitness[3]
            log_q.put(f"   - دورة {i+1}: تم العثور على حل أفضل. لياقة (نقص, صارم, مرن)=({unplaced}, {hard}, {soft_count}={soft_sum})")
        ...
        # التعديل التفاعلي الذكي لحجم الذاكرة
        if best_fitness == last_best_fitness:
            ...
            stagnation_counter = 0 
            current_tenure = max(int(base_tenure * 0.5), 2)
