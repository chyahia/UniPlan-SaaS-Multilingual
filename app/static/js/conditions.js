...
function renderConditionsUI() {
    ...
    condDays.forEach(d => thHtml += `<th>${_t(d)}</th>`);
    ...
    if (restrictedDaySelect && condDays && condDays.length > 0) {
        // ✨ لاحظ أننا تركنا value="${day}" كما هي، وغلفنا العرض فقط بـ ${_t(day)}
        restrictedDaySelect.innerHTML = condDays.map(day => `<option value="${day}">${_t(day)}</option>`).join('');
    }
