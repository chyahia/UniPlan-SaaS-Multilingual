...
def start_solver():
    ...
    # ✨ 1. استدعاء المترجم المستقل للواجهة
    from app.resit_solver import set_resit_algorithm_language 
    ...
    constraints.update({'strategy': strategy, 'duration': duration, 'destruction_rate': destruction_rate})
    ...
    if mode == 'desktop':
        ...
        def run_thread(lang):
            # ✨ 2. تفعيل المترجم المعزول داخل العملية الخلفية لسطح المكتب
            set_resit_algorithm_language(lang)
            with app_obj.app_context():
                from app.resit_tasks import execute_resit_distribution
                execute_resit_distribution(tenant_id, duration, destruction_rate, strategy, celery_task=None)
                    
        threading.Thread(target=run_thread, args=(current_lang,)).start()
    else:
        # مسار السحابة المعتاد
        task = run_resit_distribution_task.delay(tenant_id, duration, destruction_rate, strategy, current_lang)
...
def stop_solver():
    ...
    if mode == 'desktop':
        # إيقاف الخوارزمية في نسخة سطح المكتب
        from app.memory_logger import store
        with store.lock:
            store.status[f"running_{tenant_id}"] = False
            # إضافة رسالة توضح أنه تم الإيقاف يدوياً
            store.status[f"done_{tenant_id}"] = [{"error": _("تم إيقاف الخوارزمية يدوياً.")}] 
    else:
        # إيقاف الخوارزمية في النسخة السحابية (Celery)
        task_id = session.get('resit_task_id')
        if task_id:
            # استدعاء المهمة مباشرة وإلغائها بدون الحاجة للبحث عن مجلد extensions
            from app.resit_tasks import run_resit_distribution_task
            task_result = run_resit_distribution_task.AsyncResult(task_id)
...
def export_excel():
    ...
    # ✨ تجهيز أعمدة العرض (مع إضافة الترجمة في حالة اللغة الإنجليزية)
    display_columns = []
    for d in all_days:
        if lang == 'en':
            ar_day = d.split('(')[0].strip()
            en_day = day_trans.get(ar_day, ar_day)
            date_part = d[d.find('('):] if '(' in d else ""
            display_columns.append(f"{ar_day} / {en_day} {date_part}".strip())
        else:
    ...
    for level_name in all_levels:
        # ✨ استخدام الأعمدة المزينة للواجهة
        df_level = pd.DataFrame(index=all_times, columns=display_columns)
        
        # ✨ ترجمة عنوان الفترة
        df_level.index.name = "Time / الفترة" if lang == 'en' else _("الفترة")

        for idx, day in enumerate(all_days):
            display_day = display_columns[idx]
            for time_val in all_times:
                if day in final_schedule and time_val in final_schedule[day]:
                    ...
                    if level_name in levels_dict:
                        ...
                        existing = df_level.at[time_val, display_day]
                        if pd.notna(existing) and str(existing).strip() != '':
                            df_level.at[time_val, display_day] = str(existing) + "\n\n====================\n\n" + cell_content
                        else:
                            df_level.at[time_val, display_day] = cell_content
    ...
    if file.filename.endswith('.xlsx'):
        try:
            ...
            for sheet_name, df in xls.items():
                for excel_day in df.columns:
                    # ✨ تنظيف اسم اليوم لإزالة الترجمة الإنجليزية واستعادة المفتاح العربي
                    clean_day = str(excel_day)
                    if ' / ' in clean_day:
                        ar_part = clean_day.split(' / ')[0].strip()
                        date_part = clean_day[clean_day.find('('):] if '(' in clean_day else ""
                        clean_day = f"{ar_part} {date_part}".strip()
                            ...
                            for exam_block in exams_in_cell:
                                ...
                                if ':::' in clean_block:
                                    ...
                                    if len(parts) >= 5:
                                        ...
                                        # ✨ استخدام clean_day بدلاً من excel_day لبناء القاموس
                                        if clean_day not in new_schedule: new_schedule[clean_day] = {}
                                        if time_val not in new_schedule[clean_day]: new_schedule[clean_day][time_val] = {}
                                        if level not in new_schedule[clean_day][time_val]:
                                            new_schedule[clean_day][time_val][level] = {"subject": subject, "subject_teachers": teachers, "rooms": {}}
                                        ...
                                        for r_block in room_blocks:
                                            if ':' in r_block:
                                                ...
                                                new_schedule[clean_day][time_val][level]["rooms"][r_name] = guards_list
